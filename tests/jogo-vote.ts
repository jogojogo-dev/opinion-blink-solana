...
describe("jogo-vote", () => {
  ...
  before(async() => {
    const tx = await program.provider.connection.requestAirdrop(payer.publicKey, 10000000000);
  ...
  it("Initialize state", async () => {
      const tx = await program.methods.initialize(maxVoteNumbers).accounts({
  ...
  it("Multiple vote", async () => {
    for (let i = 0; i < 10; i++) {
      ...
      const tx = await program.methods.vote(randomVoteNumber).accounts({
        state: stateAccountKey,
