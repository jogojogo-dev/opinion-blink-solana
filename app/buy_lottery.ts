...
async function buyLotteryTransaction(
  voter: PublicKey,
  voteNumber: number,
  network: Network
) {
  ...
  const lotteryPoolVaultAccount = new PublicKey(
    lotteryPoolVaultAccounts[network]
  );
  ...
  const data = Buffer.concat([
    discriminator,
    Buffer.from(new Uint8Array(new BN(voteNumber).toArray("le", 8))),
  ]);
...
async function buyLottery() {
  ...
  const voteNumber = 2;
  const tx = await buyLotteryTransaction(
    payer.publicKey,
    voteNumber,
    "localnet"
  );
...
async function getUserLotteries(network: Network) {
  ...
  userLotteryAccounts.map((userLottery) => {
    const { owner, lotteryPool, balance, voteNumber, bump, isClaimed } =
      userLottery.account;
...
async function getTotalUserAtVoteNumber(network: Network, voteNumber: number) {
  ...
  return userLotteries.filter(
    (userLottery) => userLottery.account.voteNumber.toNumber() === voteNumber
  ).length;
}

async function checkUserHasVoteNumber(
  network: Network,
